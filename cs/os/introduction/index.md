...
폰 노이만이 컴퓨터의 아버지라 불리는 이유는 내장형 프로그램 방식을 처음 도입했기 때문. 메모리에 프로그램을 저장하는 컴퓨터. RAM의 명령어를 CPU가 fetch해서 execute하는 사이클 가지는 컴퓨터를 처음 설계. **폰 노이만 구조**. ISA와 명령어 집합으로 컴퓨터를 운영하는 것. 
